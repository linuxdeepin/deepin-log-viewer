...
{
    qCDebug(logApp) << "Checking if user name is valid:" << userName;
...
{
    qCDebug(logApp) << "Getting home path for user:" << userName;
    QString uName("");
    if (!userName.isEmpty())
        uName = userName;
    else
    ...
    qCDebug(logApp) << "userName: " << uName << "homePath:" << homePath;
